...
let ToSanString (board : Position) (move:ValidatedMove) :string =
    ...
    let piece = match move.Hint.Piece with | Some(t) -> t | _ -> failwith "unexpected"
    let file x = x |> fst |> printFile
    let rank x = x |> snd |> rankToString
    ...
    if move.Hint.ResultPosition.Value.Observations |> MyList.contains Check then
        sb.Append('+') |> ignore
    else if move.Hint.ResultPosition.Value.Observations |> MyList.contains Mate then
        sb.Append('#') |> ignore
