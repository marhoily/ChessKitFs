...
let IsValidPawnMove() = 
    ...
    |> after "e7-e5"
    |> should equal 
           "r1bqkb1r/pppp1ppp/2n2n2/4p3/PP6/N7/2PPPPPP/R1BQKBNR w KQkq e6 0 4"
...
let ``Black Take EnPassant``() = 
    ...
    |> after "e4-f3"
    |> should equal 
           "Nnbqkbnr/ppp3pp/8/2P2p2/8/3p1p2/PP1PP1PP/R1BQKBNR w KQk - 0 8"
...
let WhiteKingCastling() = 
    ...
    |> after "e1-g1"
    |> should equal 
           "r3k2r/p3bppp/np3n2/2p1B3/7P/1P1b3B/P1PNP2P/RNQ2RK1 b kq - 3 14"
...
let WhiteQueenCastling() = 
    ...
    |> after "e1-c1"
    |> should equal 
           "rnbk1b1r/p1p1qpp1/3pn3/1N2p1BQ/2P4P/P2P4/1P2BPPN/2KR3R b - - 10 16"
...
let ``Kill BlackQueen castling``() = 
    ...
    |> after "a8-b8"
    |> should equal 
           "1rb1kb1r/p2p3p/n6n/q1p1pp1P/1p2N1p1/PQP3PN/1P1PPP2/R1B1KBR1 w Qk - 2 13"
...
let EnPassant() = 
    ...
    |> after "f2-f4"
    |> should equal 
           "rnbqkb1r/pppppppp/7n/8/5P2/7N/PPPPP1PP/RNBQKB1R b KQkq f3 0 2"

[<Fact>]
let Castling1() = 
    "rnbqkbnr/ppp3pp/1N6/3ppp2/8/8/PPPPPPPP/R1BQKBNR w KQkq - 0 4 "
    |> after "b6-a8"
    |> should equal "Nnbqkbnr/ppp3pp/8/3ppp2/8/8/PPPPPPPP/R1BQKBNR b KQk - 0 4"
...
let Castling2() = 
    ...
    |> after "d4-a1"
    |> should equal 
           "r1bknb1r/pp1pp1pp/n1pB4/5p2/5P2/QP5N/P1PPP1PP/qN2KB1R w K - 0 11"
...
let Castling3() = 
    ...
    |> after "f2-h1"
    |> should equal 
           "rnbqkb1r/ppp2ppp/2N5/3pp3/8/8/PPPPP1PP/R1BQKBNn w Qkq - 0 7"
...
let ``Piece take should reset halfmove clock``() = 
    ...
    |> after "b7-e4"
    |> should equal 
           "1r2kbnr/p2p3p/nq4N1/2p1pp1P/1p2b1p1/PQPP2P1/1P2PPB1/R1B1K1R1 w Qk - 0 17"
...
let ``Rook take should remove castling option``() = 
    ...
    |> after "g6-h8"
    |> should equal 
           "1r2kbnN/p2p3p/nq6/2pQp2P/1p3pp1/PPPP2P1/4PPb1/R1B1K1R1 b Q - 0 19"
...
let ``King move should kill castling``() = 
    ...
    |> after "e1-f1"
    |> should equal 
           "1r2kb1N/p1Qpn3/nq5p/4p2P/1p1p1pp1/PPP3P1/4PP2/R1B2KR1 b - - 0 23"
...
let ``King move should kill castling1``() = 
    ...
    |> after "e8-d7"
    |> should equal 
           "2bq1bnr/rp1kBppp/pn1p4/2p5/1PP4P/N7/P2PPPP1/R2QKBNR w KQ - 1 9"
...
let ``Take en-passant removes check``() = 
    ...
    |> after "f5-g6"
    |> should equal 
           "rnb1nrk1/pp3p2/1q2p1P1/3p3P/2pbPK2/PB5P/1PPP4/RNBQ2NR b - - 0 22"
